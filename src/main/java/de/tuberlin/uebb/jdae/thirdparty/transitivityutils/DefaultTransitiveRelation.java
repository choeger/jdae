...
class DefaultTransitiveRelation<E> implements TransitiveRelation<E>,
    ...
    private void propagate(Node<E> subject, Node<E> object) {
        ...
        while (!toVisit.isEmpty()) {
            ...
            if (!current.intervalSet.containsAll(subject.intervalSet)) { // this
                                                                         // gracefully
                                                                         // handles
                                                                         // cycles
    ...
    private static class Node<E> {
        /*
         * We store the value of this node in pre, and whether this node is
         * enclosable in post
         */
        ...
        boolean isEnclosable() {
            ...
            if (isEnclosable) {
                isEnclosable = intervalSet.size() == 2; // otherwise, this node
                                                        // has been tainted
                // with foreign intervals, and it can never be enclosable
                // (otherwise the enclosing interval
                // would also subsume the gaps between the intervals, which
                // could lead to errors)
                if (!isEnclosable)
                    markNotEnclosable(); // also speeds up subsequent
                                         // invocations
            }
            return isEnclosable;
        }
        ...
        // but we never get E from post.getValue(), rather we only use it to
        // test
        // whether it holds null or ENCLOSABLE_MARKER. Also, post is never
        // exposed.
        Node<E> createEnclosing(DefaultTransitiveRelation<E> owner, E value) {
            OrderList.Node<E> newPre = owner.magicList.addAfter(pre.previous(),
                    value);
            OrderList.Node<E> newPost = owner.magicList.addAfter(post,
                    (E) ENCLOSABLE_MARKER);
            markNotEnclosable();
            return createAndRegister(owner, newPre, newPost, value);
        }

        // Note that in this case the created node cannot be enclosed; it is
        // created already enclosed
        Node<E> createEnclosed(DefaultTransitiveRelation<E> owner, E value) {
            OrderList.Node<E> newPre = owner.magicList.addAfter(
                    post.previous(), value);
        ...
        static <E> Node<E> create(DefaultTransitiveRelation<E> owner, E value) {
            OrderList.Node<E> newPre = owner.magicList.addAfter(owner.magicList
                    .base().previous(), value);
            OrderList.Node<E> newPost = owner.magicList.addAfter(newPre,
                    (E) ENCLOSABLE_MARKER);
            return createAndRegister(owner, newPre, newPost, value);
        }

        static <E> Node<E> createAndRegister(
                DefaultTransitiveRelation<E> owner, OrderList.Node<E> pre,
                OrderList.Node<E> post, E value) {
    ...
    private static class SerializationProxy<E> implements Serializable {
        ...
        // Writing the number of domain elements, then iterate over the domain
        // and write:
        // - the domain element
        ...
        private void readObject(ObjectInputStream s) throws IOException,
            ...
            SetMultimap<Object, Object> mm = HashMultimap
                    .create(domainCount, 2);
    ...
    static <A, B> Set<B> transformSet(final Set<A> set,
            final Function<? super A, ? extends B> transformer) {
