package de.tuberlin.uebb.jdae.thirdparty;
 ...
 * E.; Karp, Richard M. (1973),
 * "An n5/2 algorithm for maximum matchings in bipartite graphs", SIAM Journal
 * on Computing 2 (4): 225–231, doi:10.1137/0202019
 * 
 * A coarse overview of the algorithm is given in:
 * http://en.wikipedia.org/wiki/Hopcroft-Karp_algorithm
 * 
 * Note: the behavior of this class is undefined when the input isn't a
 * bipartite graph, i.e. when there are edges within a single partition!
 * 
 * @author Joris Kinable
 */

public final class HopcroftKarpBipartiteMatching<V, E> {
    ...
    private void maxMatching() {
        ...
        List<LinkedList<V>> augmentingPaths = this.getAugmentingPaths(); // Get
                                                                         // a
                                                                         // list
                                                                         // with
                                                                         // augmenting
        ...
        while (!augmentingPaths.isEmpty()) {
            ...
            augmentingPaths.addAll(this.getAugmentingPaths()); // Check whether
                                                               // there are new
                                                               // augmenting
                                                               // paths
                                                               // available
    ...
    private void symmetricDifference(LinkedList<V> augmentingPath) {
        int operation = 0;
        // The augmenting path alternatingly has an edge which is not part of
        // the matching, and an edge
        // which is part of the matching. Edges which are already part of the
        // matching are removed, the others are added.
        while (augmentingPath.size() > 0) {
            E edge = graph
                    .getEdge(augmentingPath.poll(), augmentingPath.peek());
            if (operation % 2 == 0) {
                matching.add(edge);
            } else
    ...
    private List<LinkedList<V>> getAugmentingPaths() {
        ...
        Set<V> oddLayer = new HashSet<V>(unmatchedVertices1); // Layer L0
                                                              // contains the
                                                              // unmatchedVertices1.
        Set<V> evenLayer;
        ...
        while (true) {
            ...
            // A new layer can ONLY contain vertices which are not used in the
            // previous layers
            ...
            for (V vertex : oddLayer) {
                ...
                for (V neighbor : neighbors) {
                    if (usedVertices.contains(neighbor)
                            || matching.contains(graph
                                    .getEdge(vertex, neighbor)))
                        continue;
                    else {
            ...
            // We are finished if 1. the last layer is empty or 2. if we reached
            // free vertices in partition2.
            if (evenLayer.size() == 0
            ...
            for (V vertex : evenLayer) {
                ...
                for (V neighbor : neighbors) {
                    if (usedVertices.contains(neighbor)
                            || !matching.contains(graph.getEdge(vertex,
                                    neighbor)))
                        continue;
                    else {
        ...
        // list.
        // Else, we need to generate the augmenting paths which start at free
        // vertices in
        // the even layer and end at the free vertices at the first odd layer
        // (L0).
        if (evenLayer.size() == 0) {
        ...
        // Finally, do a depth-first search, starting on the free vertices in
        // the last even layer.
        ...
        for (V vertex : evenLayer) {
            ...
            // remove the vertices
            // from the map to enforce that the paths are vertex disjoint, i.e.
            // a vertex cannot occur in
            // more than 1 path.
