...
/**
 * An interval set that supports adding intervals and testing whether a node
...
final class MergingIntervalSet {
    ...
    /**
     * Adds an interval to this interval set. The internal representation always
     * remains minimal and sorted, thus has a O(logn) query time.
     */
    void addInterval(Node<?> pre, Node<?> post) {
        Preconditions.checkState(pre.precedes(post),
                "Pre node does not precede post node");
        /*
         ...
         */
        int preIndex = Arrays.binarySearch(array, 0, size, pre,
                NodeComparator.INSTANCE);
        int postIndex = Arrays.binarySearch(array, 0, size, post,
                NodeComparator.INSTANCE);
        ...
        if ((preIndex & 1) != 0 || (postIndex & 1) != 0) { // overlapping
                                                           // interval
            if (preIndex == postIndex)
                return; // the interval is subsumed

            if ((preIndex & 1) != 0) { // there is another pre on the left of
                                       // the new pre; keep that
                preIndex -= 1;
                pre = array[preIndex];
            }
            if ((postIndex & 1) != 0) { // there is another post on the right of
                                        // the new post; keep that
    ...
    private void insertInterval(int preIndex, Node<?> pre, int postIndex,
            Node<?> post) {
        if (preIndex + 2 == postIndex) {
            // no copying required, just replace the respective nodes (the new
            // nodes subsume exactly one interval)
            array[preIndex] = pre;
            ...
            if (nextPowerOfTwo != newSize)
                nextPowerOfTwo <<= 1;

            if (size != nextPowerOfTwo) { // resizing if necessary
                newArray = new Node<?>[nextPowerOfTwo];
            }
            if (array != newArray) { // copying the prefix till the inserted
                                     // interval
            ...
            System.arraycopy(array, postIndex, newArray, preIndex + 2, size
                    - postIndex);
            if (newSize < size && newArray == array) { // clearing up dead
                                                       // references
     ...
     */
    private static final int BINARY_SEARCH_CUTOFF_POINT = 8; // slightly
                                                             // conservative. 10
                                                             // would be
                                                             // probably OK too

    /**
     * Tests whether a node is contained in any interval (or defines an interval
     * boundary) of this interval set.
     */
    ...
    boolean contains_binarySearch(Node<?> node) {
        int index = Arrays.binarySearch(array, 0, size, node,
                NodeComparator.INSTANCE);
        return index > 0 || // node exists as-is in the set
                (index & 1) == 0; // node does not exist, but is inside an
                                  // interval, not outside
    }

    /**
     * Tests whether this interval set contains every interval of another
     * interval set (of course, the nodes of both interval sets must belong to
     * the same instance of OrderList).
     */
        ...
        for (int thatIndex = 0; thatIndex < other.size; thatIndex += 2) {
            ...
            while (true) {
                ...
                if (thisIndex == this.size) {
                    return false; // could not find an interval that contains
                                  // otherLeft and otherRight
                }
